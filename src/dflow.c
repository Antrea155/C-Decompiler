...
void analyze_inst(Instruction *ins, BasicBlock *bb)
{
  ...
  { // push callee func parms to stack. for x86-32

    if (op1type2 == OP_TYPE_SBR)
      ; // ignore pushing the stack base reg
  ...
  else if (insGrpId == INS_GRP_MOV)
  {
    ...
    {
      if ((op2type2 == OP_TYPE_ARG) && (strlen(ins->mnemonic) == 4))
      { // x86-64 places pars for callee in edi, esi,..
        reg = cpy_sym(get_symbAt(ins->operands[0].value.reg, 0));
        push(reg);
    ...
    else if ((op1type2 == OP_TYPE_LABEL) && (op2type2 == OP_TYPE_ARG))
    { // x86-64 places pars for callee in edi, esi,..
      char *string = get_stringLabel(ins->operands[0].value.imm);
      symbol = (Symbol *)calloc(1, sizeof(Symbol));
      symbol->value = strdup(string);
  ...
  else if (insGrpId == INS_GRP_OP)
  { // add, sub, and, shr, shl, sal, sar
    ...
    {
      reg = get_symbAt(ins->operands[1].value.reg, 0);
      if (!strcmp(op, ">>") && (!strcmp(ins->operands[0].value.imm, "2"))) // sar 2 reg -> reg = reg/2
    ...
    {
      dlprintf(1, "reg1=%s reg2=%s\n", ins->operands[0].value.reg, ins->operands[1].value.reg);
      reg = get_symbAt(ins->operands[0].value.reg, 0);
