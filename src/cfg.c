...
void create_CFG(List *funcblocks) {
  ...
  // add to predecessors list of each bb the bb pointers of the bbs that connect
  // to it
  ...
  for (int i = 0; i < funcblocks->numItems; i++) {
    ...
    // delete empty blocks
    // if a bb is pointing to an empty bb then delete it and make the bb point
    // to the successor of the deleted bb
    ...
    for (int i = 0; i < BBlist->numItems; i++) {
      ...
      if (bb->thenBB) {
        ...
        if ((!targetBlock->compOperator) &&
            (List_is_empty(&(targetBlock->Instructions)))) {
...
void add_dot_link(BasicBlock *bb, BasicBlock *target, int pos, FILE *fp) {
  if (pos == 0)
    fprintf(fp, "%s%s%s%s%s\n", "\"", bb->label, "\":f1 -> \"", target->label,
            "\":f0 [");
  else
    fprintf(fp, "%s%s%s%s%s\n", "\"", bb->label, "\":f2 -> \"", target->label,
            "\":f0 [");
  fprintf(fp, "%s\n", "id=1");
  fprintf(fp, "%s\n", "];");
}
...
/* generate the control flow graph by connecting each dot node */
