...
// d:\andrea\project\dot>dot -Tsvg cfgimages.dot -o cfgimages.svg
...
void add_dot_node(BasicBlock *bb, FILE *fp) {
    ...
    if (ins == NULL) { if (bb->compOperator) strcpy(firstInstr,bb->compOperator); else strcpy(firstInstr,"empty"); } 
    else strcpy(firstInstr, ins->mnemonic);
    fprintf(fp,"%s%s%s\n", "\"",bb->label,"\"[");
